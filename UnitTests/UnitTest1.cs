...
namespace UnitTests
{
    ...
    public class UnitTest1
    {
        ...
        [TestMethod]
        public void Indicate_Selected_Genre()
        {
            //организация (arrange)
            Mock<IBookRepository> mock = new Mock<IBookRepository>();
            mock.Setup(m => m.Books).Returns(new List<Book>
            ...
            string genereToSelect = "Genre2";
            //действие (act)
            string result = target.Menu(genereToSelect).ViewBag.SelectedGenre;

            //утверждение (assert)
            Assert.AreEqual(genereToSelect, result);
        }
