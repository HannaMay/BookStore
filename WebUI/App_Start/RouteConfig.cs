...
namespace WebUI
{
    public class RouteConfig
    {
        public static void RegisterRoutes(RouteCollection routes)
        {
            ...
            routes.MapRoute(
                null,
                ResolveGenreForUrl(mygenre),
                new { controller = "Books", action = "List", page = 1 }
            );

            routes.MapRoute(
                null,
                ResolveGenreForUrl(mygenre).ToString() + "/Page{page}",
        ...
        public static string ResolveGenreForUrl(string genre) => Regex.Replace(Regex.Replace(genre, "[^\\w]", "-"), "[-]{2,}", "-");
